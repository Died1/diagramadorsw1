 ...
 */
Menus = function Menus(editorUi) {
  console.log(editorUi);
  this.editorUi = editorUi;
  this.menus = new Object();
  this.init(); // Pre-fetches checkmark image
...
Menus.prototype.init = function () {
    ...
    var addItem = mxUtils.bind(this, function (fontsize) {
      this.styleChange(menu, fontsize, [mxConstants.STYLE_FONTSIZE], [fontsize], null, parent, function () {
        if (graph.cellEditor.textarea != null) {
          ...
          document.execCommand('fontSize', false, '3'); // Changes the css font size of the first font element inside the in-place editor with size 3
  ...
  this.put('file', new Menu(mxUtils.bind(this, function (menu, parent) {
    this.addMenuItems(menu, ['new', 'open', 'save', 'saveAs', 'import', 'export', 'generateCode'], parent);
  })));
...
Menus.prototype.addInsertTableCellItem = function (menu, parent) {
  ...
  this.addInsertTableItem(menu, mxUtils.bind(this, function (evt, rows, cols) {
    var table = mxEvent.isControlDown(evt) || mxEvent.isMetaDown(evt) ? graph.createCrossFunctionalSwimlane(rows, cols) : graph.createTable(rows, cols, null, null, mxEvent.isShiftDown(evt) ? 'Table' : null);
...
Menus.prototype.addInsertTableItem = function (menu, insertFn, parent) {
  insertFn = insertFn != null ? insertFn : mxUtils.bind(this, function (evt, rows, cols) {
    ...
    if (td != null && graph.cellEditor.textarea != null) {
      var row2 = graph.getParentByName(td, 'TR'); // To find the new link, we create a list of all existing links first
      ...
      graph.pasteHtmlAtCaret(createTable(rows, cols)); // Moves cursor to first table cell
  ...
  var elt2 = menu.addItem('', null, null, parent, null, null, null, true); // Quirks mode does not add cell padding if cell is empty, needs good old spacer solution
  ...
  mxEvent.addGestureListeners(picker, null, null, mxUtils.bind(this, function (e) {
    ...
    if (td != null && row2 != null && selected) {
      insertFn(e, row2.sectionRowIndex + 1, td.cellIndex + 1); // Async required to block event for elements under menu
    ...
    try {
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        if (graph.getModel().isEdge(cell)) {
          if (reset) {
            var geo = graph.getCellGeometry(cell); // Resets all edge points
  ...
  return mxUtils.bind(this, function (post) {
    ...
    try {
      ...
      for (var i = 0; i < keys.length; i++) {
        graph.setCellStyles(keys[i], values[i], cells); // Removes CSS alignment to produce consistent output
...
Menus.prototype.addMenuItem = function (menu, key, parent, trigger, sprite, label) {
  ...
  if (action != null && (menu.showDisabled || action.isEnabled()) && action.visible) {
    ...
    }, parent, sprite, action.isEnabled()); // Adds checkmark image
...
Menus.prototype.addMenuItems = function (menu, keys, parent, trigger, sprites) {
  for (var i = 0; i < keys.length; i++) {
    if (keys[i] == '-') {
      ...
      if (keys[i] === 'import') {
        console.log("agregando items", parent);
      }
  ...
  if (state != null) {
    ...
    if (graph.getModel().isEdge(cell) && mxUtils.getValue(state.style, mxConstants.STYLE_EDGE, null) != 'entityRelationEdgeStyle' && mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null) != 'arrow') {
      ...
      if (handler instanceof mxEdgeHandler && handler.bends != null && handler.bends.length > 2) {
        var index = handler.getHandleForEvent(graph.updateMouseEvent(new mxMouseEvent(evt))); // Configures removeWaypoint action before execution
      ...
      this.addMenuItems(menu, [isWaypoint ? 'removeWaypoint' : 'addWaypoint'], null, evt); // Adds reset waypoints option if waypoints exist
    ...
    if (graph.getSelectionCount() == 1) {
      this.addMenuItems(menu, ['-', 'editStyle', 'editData', 'editLink'], null, evt); // Shows edit image action if there is an image in the style
...
Menubar.prototype.addMenuHandler = function (elt, funct) {
  if (funct != null) {
    ...
    mxEvent.addListener(elt, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
...
Menubar.prototype.destroy = function () {// do nothing
};
/**
