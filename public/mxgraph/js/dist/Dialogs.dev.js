...
var OpenDialog = function OpenDialog() {
  ...
  iframe.frameBorder = '0'; // Adds padding as a workaround for box model in older IE versions
...
var ColorDialog = function ColorDialog(editorUi, color, apply, cancelFn) {
  ...
  input.style.width = '216px'; // Required for picker to render in IE
  ...
  mxUtils.br(div); // Adds recent colors

  createRecentColorTable(); // Adds presets
  ...
  this.picker = picker;
  this.colorInput = input; // LATER: Only fires if input if focused, should always
...
var AboutDialog = function AboutDialog(editorUi) {
  var div = document.createElement('div');
...
var TextareaDialog = function TextareaDialog(editorUi, title, url, fn, cancelFn, cancelTitle, w, h, addButtons, noHide, noWrap, applyTitle, helpLink, customButtons) {
  w = w != null ? w : 300;
  h = h != null ? h : 120;
...
var EditDiagramDialog = function EditDiagramDialog(editorUi) {
  ...
  var okBtn = mxUtils.button(mxResources.get('ok'), function () {
    ...
    if (select.value == 'new') {
      ...
      try {
        editorUi.editor.setGraphXml(mxUtils.parseXml(data).documentElement); // LATER: Why is hideDialog between begin-/endUpdate faster?
      ...
      try {
        ...
        editorUi.editor.graph.setSelectionCells(editorUi.editor.graph.importCells(children)); // LATER: Why is hideDialog between begin-/endUpdate faster?
...
var ExportDialog = function ExportDialog(editorUi) {
  ...
  row.appendChild(td);
  tbody.appendChild(row);
  table.appendChild(tbody); // Handles changes in the export format
...
ExportDialog.exportFile = function (editorUi, name, format, bg, s, b, dpi) {
  ...
  if (format == 'xml') {
    ...
    var bounds = graph.getGraphBounds(); // New image export

    var xmlDoc = mxUtils.createXmlDocument();
    var root = xmlDoc.createElement('output');
    xmlDoc.appendChild(root); // Renders graph. Offset will be multiplied with state's scale when painting state.
    ...
    imgExport.drawState(graph.getView().getState(graph.model.root), xmlCanvas); // Puts request data together
    ...
    console.log('kjldj', mxUtils.getXml(root)); // Requests image if request is valid

    if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
      editorUi.hideDialog();
      var req = new mxXmlRequest(EXPORT_URL, 'format=' + format + '&filename=' + encodeURIComponent(name) + '&bg=' + (bg != null ? bg : 'none') + '&w=' + w + '&h=' + h + '&' + param + '&dpi=' + dpi);
...
var EditDataDialog = function EditDataDialog(ui, cell) {
  ...
  var value = graph.getModel().getValue(cell); // Converts the value to an XML node
  ...
  var addBtn = mxUtils.button(mxResources.get('addProperty'), function () {
    var name = nameInput.value; // Avoid ':' in attribute names which seems to be valid in Chrome
  ...
  var applyBtn = mxUtils.button(mxResources.get('apply'), function () {
    try {
      ui.hideDialog.apply(ui, arguments); // Clones and updates the value
  ...
  ;
  mxEvent.addListener(nameInput, 'keyup', updateAddBtn); // Catches all changes that don't fire a keyup (such as paste via mouse)
...
var LinkDialog = function LinkDialog(editorUi, initialValue, btnLabel, fn) {
  ...
  cross.style.cursor = 'pointer'; // Workaround for inline-block not supported in IE

  cross.style.display = mxClient.IS_VML ? 'inline' : 'inline-block';
  cross.style.top = (mxClient.IS_VML ? 0 : 3) + 'px'; // Needed to block event transparency in IE
  ...
  mxEvent.addListener(removeLink, 'click', function (evt) {
    if (graph.isEnabled()) {
      ...
      try {
        ...
        graph.removeCells([selectionLayer], false); // Creates default layer if no layer exists
  ...
  function refresh() {
    ...
    function addLayer(index, label, child, defaultParent) {
      ...
      mxEvent.addListener(ldiv, 'dragstart', function (evt) {
        dragSource = ldiv; // Workaround for no DnD on DIV in FF
      ...
      if (graph.isEnabled()) {
        ...
        if (mxClient.IS_TOUCH || mxClient.IS_POINTER || mxClient.IS_VML || mxClient.IS_IE && document.documentMode < 10) {
          ...
          right.style.top = '6px'; // Poor man's change layer order
