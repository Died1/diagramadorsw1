 ...
 */
EditorUi = function EditorUi(editor, container, lightbox) {
  ...
  this.initialDefaultEdgeStyle = mxUtils.clone(graph.defaultEdgeStyle); // Faster scrollwheel zoom is possible with CSS transforms
  ...
  if (!graph.standalone) {
    this.createDivs();
    this.createUi();
    this.refresh(); // Disables HTML and text selection
    ...
    if (!this.editor.chromeless || this.editor.editable) {
      ...
      var linkHandler = function linkHandler(evt) {
        if (evt != null) {
    ...
    graph.init(this.diagramContainer); // Improves line wrapping for in-place editor
    ...
    this.hoverIcons = this.createHoverIcons(); // Hides hover icons when cells are moved
    ...
    var spaceKeyPressed = false; // Overrides hovericons to disable while space key is pressed
    ...
    this.keydownHandler = mxUtils.bind(this, function (evt) {
      ...
      && !graph.isEditing()) {
        ...
        graph.container.style.cursor = "move"; // Disables scroll after space keystroke with scrollbars
    ...
    mxEvent.addListener(document, "keyup", this.keyupHandler); // Forces panning for middle and right mouse buttons
    ...
    var ui = this; // Overrides cell editor to update toolbar
    ...
    graph.container.style.cursor = "default"; // Workaround for page scroll if embedded via iframe

    if (window.self === window.top && graph.container.parentNode != null) {
      try {
        graph.container.focus();
      } catch (e) {// ignores error in old versions of IE
      }
    ...
    graph.popupMenuHandler.autoExpand = true; // Installs context menu
    ...
    this.keyHandler = this.createKeyHandler(editor); // Getter for key handler
    ...
    var styles = ["rounded", "shadow", "glass", "dashed", "dashPattern", "labelBackgroundColor", "comic", "sketch", "fillWeight", "hachureGap", "hachureAngle", "jiggle", "disableMultiStroke", "disableMultiStrokeFill", "fillStyle", "curveFitting", "simplification", "sketchStyle"];
    var connectStyles = ["shape", "edgeStyle", "curved", "rounded", "elbow", "jumpStyle", "jumpSize", "comic", "sketch", "fillWeight", "hachureGap", "hachureAngle", "jiggle", "disableMultiStroke", "disableMultiStrokeFill", "fillStyle", "curveFitting", "simplification", "sketchStyle"]; // Note: Everything that is not in styles is ignored (styles is augmented below)
    ...
    this.clearDefaultStyle = function () {
      ...
      graph.currentVertexStyle = mxUtils.clone(graph.defaultVertexStyle); // Updates UI
    ...
    var valueStyles = ["fontFamily", "fontSource", "fontSize", "fontColor"]; // Keys that always update the current edge style regardless of selection

    var alwaysEdgeStyles = ["edgeStyle", "startArrow", "startFill", "startSize", "endArrow", "endFill", "endSize"]; // Keys that are ignored together (if one appears all are ignored)

    var keyGroups = [["startArrow", "startFill", "startSize", "sourcePerimeterSpacing", "endArrow", "endFill", "endSize", "targetPerimeterSpacing"], ["strokeColor", "strokeWidth"], ["fillColor", "gradientColor"], valueStyles, ["opacity"], ["align"], ["html"]]; // Adds all keys used above to the styles array
    ...
    var insertHandler = function insertHandler(cells, asText, model) {
    ...
    this.addListener("styleChanged", mxUtils.bind(this, function (sender, evt) {
      ...
      for (var i = 0; i < keys.length; i++) {
        var common = mxUtils.indexOf(valueStyles, keys[i]) >= 0; // Ignores transparent stroke colors
    ...
    mxEvent.addGestureListeners(document, this.gestureHandler); // Updates the editor UI after the window has been resized or the orientation changes
    ...
    mxEvent.addListener(window, "orientationchange", this.orientationChangeHandler); // Workaround for bug on iOS see
...
EditorUi.prototype.installShapePicker = function () {
  var graph = this.editor.graph;
  var ui = this; // Uses this event to process mouseDown to check the selection state before it is changed
  ...
  graph.addListener(mxEvent.DROP, mxUtils.bind(this, function () {
    ui.hideShapePicker(true);
  }));
  graph.getModel().addListener(mxEvent.CHANGE, mxUtils.bind(this, function () {
    ui.hideShapePicker(true);
  ...
  graph.dblClick = function (evt, cell) {
    if (this.isEnabled()) {
      if (cell == null && ui.sidebar != null && !mxEvent.isShiftDown(evt)) {
        mxEvent.consume(evt);
        var pt = mxUtils.convertPoint(this.container, mxEvent.getClientX(evt), mxEvent.getClientY(evt)); // Asynchronous to avoid direct insert after double tap
...
EditorUi.prototype.showShapePicker = function (x, y, source, callback, direction) {
  ...
  if (cells != null && cells.length > 0) {
    ...
    var style = source != null && (sourceState == null || !graph.isTransparentState(sourceState)) ? graph.copyStyle(source) : null; // Do not place entry under pointer for touch devices
...
EditorUi.prototype.onKeyDown = function (evt) {
  var graph = this.editor.graph; // Tab selects next cell
...
EditorUi.prototype.onKeyPress = function (evt) {
  var graph = this.editor.graph; // KNOWN: Focus does not work if label is empty in quirks mode
  ...
  if (this.isImmediateEditingEvent(evt) && !graph.isEditing() && !graph.isSelectionEmpty() && evt.which !== 0 && evt.which !== 27 && !mxEvent.isAltDown(evt) && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt)) {
    graph.escape();
    graph.startEditing(); // Workaround for FF where char is lost if cursor is placed before char

    if (mxClient.IS_FF) {
      var ce = graph.cellEditor;

      if (ce.textarea != null) {
        ce.textarea.innerHTML = String.fromCharCode(evt.which); // Moves cursor to end of textarea
...
EditorUi.prototype.initClipboard = function () {
  ...
  mxClipboard.copy = function (graph) {
    ...
    if (graph.cellEditor.isContentEditing()) {
      ...
      var clones = graph.cloneCells(result, null, cloneMap); // Uses temporary model to force new IDs to be assigned
      ...
      for (var i = 0; i < clones.length; i++) {
        model.add(parent, clones[i]); // Checks for orphaned relative children and makes absolute
...
EditorUi.prototype.initCanvas = function () {
  ...
  if (this.editor.isChromelessView()) {
    resize = mxUtils.bind(this, function (autoscale, maxScale, cx, cy) {
      if (graph.container != null && !graph.isViewer()) {
        ...
        var s = graph.view.scale; // Normalizes the bounds
    ...
    this.chromelessResize = resize; // Hook for subclassers for override
    ...
    if (urlParams["toolbar"] != "0") {
      ...
      addButton(mxUtils.bind(this, function (evt) {
        ...
        mxEvent.consume(evt);
      }), Editor.actualSizeLargeImage, mxResources.get("fit")); // Changes toolbar opacity on hover
      ...
      if (urlParams["layers"] == "1") {
        ...
        var layersButton = addButton(mxUtils.bind(this, function (evt) {
          if (this.layersDialog != null) {
            ...
            this.layersDialog.style.bottom = parseInt(this.chromelessToolbar.style.bottom) + this.chromelessToolbar.offsetHeight + 4 + "px"; // Puts the dialog on top of the container z-index
          ...
          mxEvent.consume(evt);
        }), Editor.layersLargeImage, mxResources.get("layers")); // Shows/hides layers button depending on content
    ...
    graph.view.validate = function () {
      if (this.graph.container != null && mxUtils.hasScrollbars(this.graph.container)) {
        ...
        var size = this.graph.getPageSize(); // Updating scrollbars here causes flickering in quirks and is not needed
    ...
    if (!graph.isViewer()) {
      ...
      graph.sizeDidChange = function () {
        if (this.container != null && mxUtils.hasScrollbars(this.container)) {
          ...
          var size = this.getPageSize(); // Updates the minimum graph size

          var minw = Math.ceil(2 * pad.x + pages.width * size.width);
          var minh = Math.ceil(2 * pad.y + pages.height * size.height);
          var min = graph.minimumGraphSize; // LATER: Fix flicker of scrollbar size in IE quirks mode
          ...
          if (!this.autoTranslate && (this.view.translate.x != dx || this.view.translate.y != dy)) {
            ...
            this.view.y0 = pages.y; // NOTE: THIS INVOKES THIS METHOD AGAIN. UNFORTUNATELY THERE IS NO WAY AROUND THIS SINCE THE
            // BOUNDS ARE KNOWN AFTER THE VALIDATION AND SETTING THE TRANSLATE TRIGGERS A REVALIDATION.
            // SHOULD MOVE TRANSLATE/SCALE TO VIEW.

            var tx = graph.view.translate.x;
            var ty = graph.view.translate.y;
            graph.view.setTranslate(dx, dy); // LATER: Fix rounding errors for small zoom
  ...
  var scheduleZoom = function scheduleZoom(delay) {
      ...
      if (!graph.isMouseDown || forcedZoom) {
        updateZoomTimeout = window.setTimeout(mxUtils.bind(this, function () {
          if (graph.isFastZoomEnabled()) {
            ...
            bgGroup.style.transformOrigin = ""; // Workaround for no reset of transform in Safari
  ...
  }), graph.container); // Uses fast zoom for pinch gestures on iOS
...
EditorUi.prototype.createTemporaryGraph = function (stylesheet) {
  ...
  graph.setEnabled(false); // Container must be in the DOM for correct HTML rendering
...
EditorUi.prototype.addChromelessClickHandler = function () {
  var hl = urlParams["highlight"]; // Adds leading # for highlight color code
...
EditorUi.prototype.open = function () {
  ...
  this.editor.graph.view.validate(); // Required only in special cases where an initial file is opened
...
EditorUi.prototype.showPopupMenu = function (fn, x, y, evt) {
  ...
  menu.autoExpand = true; // Disables autoexpand and destroys menu when hidden

  menu.hideMenu = mxUtils.bind(this, function () {
    mxPopupMenu.prototype.hideMenu.apply(menu, arguments);
    menu.destroy();
  });
  menu.popup(x, y, null, evt); // Allows hiding by clicking on document
...
EditorUi.prototype.getUrl = function (pathname) {
  ...
  var parms = href.indexOf("?") > 0 ? 1 : 0; // Removes template URL parameter for new blank diagram
  ...
  if (!this.editor.extendCanvas) {
    ...
    if (mxUtils.hasScrollbars(graph.container)) {
      if (graph.pageVisible) {
        ...
        graph.container.scrollLeft = Math.floor(Math.min(pad.x, (graph.container.scrollWidth - graph.container.clientWidth) / 2)) - 1; // Scrolls graph to visible area
...
EditorUi.prototype.setPageVisible = function (value) {
  ...
  graph.view.validateBackground(); // Workaround for possible handle offset
...
function ChangePageSetup(ui, color, image, format, pageScale) {
  ...
  this.previousPageScale = pageScale; // Needed since null are valid values for color and image
...
EditorUi.prototype.addUndoListener = function () {
  ...
  undoMgr.addListener(mxEvent.CLEAR, undoListener); // Overrides cell editor to update action states
...
EditorUi.prototype.updateActionStates = function () {
  ...
  this.actions.get("removeFromGroup").setEnabled(oneVertexSelected && graph.getModel().isVertex(graph.getModel().getParent(graph.getSelectionCell()))); // Updates menu states
...
EditorUi.prototype.createDivs = function () {
  ...
  this.hsplit.setAttribute("title", mxResources.get("collapseExpand")); // Sets static style for containers
...
EditorUi.prototype.createUi = function () {
  ...
  if (this.menubar != null) {
    ...
    var contenedorMenu = document.querySelector("#contenedor-menu"); // Asignar la clase 'graph-container' al div

    header.className = "header"; // Agregar el div al cuerpo del documento

    contenedorMenu.appendChild(this.menubarContainer);
    this.container.appendChild(header);
    console.log("agregando menu", this.menubarContainer);
  } // Creates the sidebar
...
EditorUi.prototype.addSplitHandler = function (elt, horizontal, dx, onChange) {
  ...
  var last = null; // Disables built-in pan and zoom in IE10 and later
  ...
  var getValue = mxUtils.bind(this, function () {
    var result = parseInt(horizontal ? elt.style.left : elt.style.bottom); // Takes into account hidden footer
...
EditorUi.prototype.extractGraphModelFromEvent = function (evt) {
  ...
  if (evt != null) {
    ...
    if (provider != null) {
      ...
      if (data != null) {
        data = Graph.zapGremlins(mxUtils.trim(data)); // Tries parsing as HTML document with embedded XML
...
EditorUi.prototype.createKeyHandler = function (editor) {
  ...
  var queue = [];
  var thread = null; // Helper function to move cells with the cursor keys
