...
Actions.prototype.init = function () {
  ...
  var isGraphEnabled = function isGraphEnabled() {
    console.log(Action.prototype.isEnabled.apply(this, arguments) && graph.isEnabled());
  ...
  this.addAction('open...', function () {
    ...
    window.openKey = 'open'; // Closes dialog after open
    ...
    window.openFile.setConsumer(mxUtils.bind(this, function (xml, filename) {
      ...
      } catch (e) {//mxUtils.alert(mxResources.get('invalidOrMissingFile') + ': ' + e.message);
      }
  ...
  this.addAction('import...', function () {
    window.openNew = false;
    window.openKey = 'import'; // Closes dialog after open
  ...
  this.addAction('save', function () {
    guardarArchivo(ui, false); //ui.saveFile(false);
  ...
  this.addAction('saveAs...', function () {
    try {
      guardarArchivo(ui, true); //ui.saveFile(true);
    } catch (error) {
      console.log(error);
    }
  ...
  this.addAction('undo', function () {
    console.log(ui.editor.undoManager);
    ui.undo();
  ...
  }, null, null, Editor.ctrlKey + '+L'); // Navigation actions
  ...
  }, null, null, Editor.ctrlKey + '+End'); // Arrange actions
  ...
  this.addAction('ungroup', function () {
    if (graph.isEnabled()) {
      ...
      try {
        var temp = graph.ungroupCells(); // Clears container flag for remaining cells
  ...
  this.addAction('removeFromGroup', function () {
    if (graph.isEnabled()) {
      var cells = graph.getSelectionCells(); // Removes table rows and cells
  ...
  this.addAction('link...', mxUtils.bind(this, function () {
    if (graph.isEnabled()) {
      if (graph.cellEditor.isContentEditing()) {
        ...
        var oldValue = ''; // Workaround for FF returning the outermost selected element after double
  ...
  action.isEnabled = isGraphEnabled;
  action.visible = false; // Help actions
  ...
  var toggleFontStyle = mxUtils.bind(this, function (key, style, fn, shortcut) {
    return this.addAction(key, function () {
      if (fn != null && graph.cellEditor.isContentEditing()) {
        ...
        try {
          var cells = graph.getSelectionCells();
          graph.toggleCellStyleFlags(mxConstants.STYLE_FONTSTYLE, style, cells); // Removes bold and italic tags and CSS styles inside labels
  ...
  }, Editor.ctrlKey + '+U'); // Color actions
        ...
        if (graph.cellEditor.isContentEditing()) {
          ...
          if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
            ...
            try {
              ...
              if (cells.length == 0) {
                cells = [graph.insertVertex(graph.getDefaultParent(), null, '', 0, 0, w, h, 'shape=image;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;')];
              ...
              graph.setCellStyles(mxConstants.STYLE_IMAGE, newValue.length > 0 ? newValue : null, cells); // Sets shape only if not already shape with image (label or image)
