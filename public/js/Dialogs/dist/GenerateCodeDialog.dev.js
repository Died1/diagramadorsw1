...
function generateAndSave(editorUi, fileName, language) {
  var ui = editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var model = graph.getModel(); // Obtiene todas las celdas del modelo

  var cells = model.cells;
  console.log(cells);
  var xml = mxUtils.getXml(editor.getGraphXml());
  console.log(xml);
  axios.post('/room/gen_code', {
    firstName: 'Fred',
    lastName: 'Flintstone'
...
var GenerateCodeDialog = function GenerateCodeDialog(editorUi) {
  ...
  tbody.appendChild(row);
  table.appendChild(tbody);
  this.container = table; // Handles changes in the export format
